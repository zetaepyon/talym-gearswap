...
function init_job_states(job_bools,job_modes)
    ...
    local x,y = settings["ui_x_res"]-315, settings["ui_y_res"]-140 -- -285, -18

    stateBox = texts.new()
    stateBox:pos(x,y)
    stateBox:font('Play')--Arial
    stateBox:size(12)
    stateBox:bold(true)
...
function update_job_states()
    ...
    for i,n in ipairs(stateList) do
        ...
        if state.HybridMode and n == 'OffenseMode' and state.HybridMode.current ~= state.HybridMode[1] then
            stateBox:append(string.format("%s / %s%s%s", clr.n, clr.h, state.HybridMode.current, clr.n))
        elseif n == 'OffenseMode' and state.DefenseMode.current ~= state.DefenseMode[1] then
            stateBox:append(string.format("%s / %s%s%s", clr.n, clr.h, state.DefenseMode.current, clr.n))
        end
